      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-extrabold text-slate-800">Local LLM Host</h3>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <select
              ...
              className="w-full mt-1 px-3 py-2 text-xs font-semibold text-slate-800 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#FF4D4D] cursor-pointer"
              id="local-provider-select"
            >
              <option value="ollama">Ollama (Default Port 11434)</option>
              <option value="lm-studio">LM Studio (Default Port 1234)</option>
              <option value="llama-cpp">Llama.cpp (Default Port 8080)</option>
          ...
          <div className="col-span-2">
            ...
            <div className="flex gap-1.5 mt-1">
              <input
                ...
                className="flex-1 px-3 py-2 text-xs font-mono font-medium text-slate-800 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#FF4D4D]"
                id="local-url-input"
              />
              <button
                type="button"
                ...
                disabled={isTesting}
                className="px-3 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl flex items-center gap-1 transition-colors cursor-pointer shrink-0"
        ...
        <div className="space-y-1">
          ...
          {localModels.length > 0 ? (
            <select
              ...
              className="w-full px-3 py-2 mt-1 text-xs font-mono font-medium text-slate-800 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#FF4D4D] cursor-pointer"
            ...
            <div className="p-2.5 border border-dashed border-slate-200 bg-white text-[11px] text-slate-500 rounded-xl text-center italic">
          ...
          <AnimatePresence>
            {showTroubleshoot && (
              <motion.div
                ...
                className="overflow-hidden text-[11px] text-slate-500 space-y-2 mt-2 pt-2 border-t border-slate-100"
              ...
              >
                ...
                <div className="space-y-1.5 pl-2 font-mono text-[10px] text-slate-600">
                  <div className="p-1.5 bg-slate-100 rounded-md">
                    <strong className="text-slate-800 block text-[9px] uppercase font-sans font-bold">Ollama (macOS/Linux)</strong>
                    OLLAMA_ORIGINS="*" ollama serve
                  </div>
                  <div className="p-1.5 bg-slate-100 rounded-md">
                    <strong className="text-slate-800 block text-[9px] uppercase font-sans font-bold">Ollama (Windows Environment)</strong>
                    Set System Variable <code className="bg-slate-200 px-1 rounded text-[#FF4D4D]">OLLAMA_ORIGINS=*</code>, then restart Ollama.
                  </div>
                  <div className="p-1.5 bg-slate-100 rounded-md">
                    <strong className="text-slate-800 block text-[9px] uppercase font-sans font-bold">LM Studio</strong>
      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-extrabold text-slate-800">Wi-Fi Network Scanner</h3>
          </div>
          {isScanning && (
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <input
              ...
              className="w-full mt-1 px-3 py-1.5 text-xs font-mono text-slate-800 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#FF4D4D]"
          ...
          <div className="space-y-1.5" id="found-llms-list">
            ...
            <div className="space-y-1.5 max-h-36 overflow-y-auto">
              {foundServices.map((srv, index) => (
                <div 
                  key={index}
                  className="p-2.5 bg-white border border-slate-200 rounded-xl flex items-center justify-between text-xs transition-all hover:border-[#FF4D4D]"
                >
                  <div className="flex items-center gap-2">
                    <Database className="w-4 h-4 text-emerald-500" />
                    <div>
                      <p className="font-extrabold text-slate-800 font-mono text-[11px]">{srv.ip}:{srv.port}</p>
                      <p className="text-[10px] text-slate-400 capitalize font-medium">{srv.provider} • {srv.modelCount} models</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => handleConnectToFound(srv.ip, srv.port, srv.provider)}
                    className="p-1 px-2.5 bg-slate-900 text-white text-[10px] font-extrabold rounded-lg hover:bg-[#FF4D4D] transition-colors flex items-center gap-0.5 cursor-pointer"
                  >
